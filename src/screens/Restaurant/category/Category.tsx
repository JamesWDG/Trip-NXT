...
const Category = ({ navigation, route }: { navigation: any; route?: any }) => {
    ...
    const [getAccomodationItemsByCategory] = useLazyGetAccomodationItemsByCategoryQuery();
    ...
    const fetchItemsByCategory = async () => {
        try {
            setLoading(true);
            const res = await getItemsByCategory({ category: route.params?.category, restaurant: route.params?.restaurant }).unwrap();
    ...
    const fetchAccomodationItemsByCategory = async () => {
        try {
            setLoading(true);
            const res = await getAccomodationItemsByCategory({ type: route.params?.category.toLowerCase() as 'standard' | 'luxury' | 'budget' }).unwrap();
            setItems(res.data);
        } catch (error) {
            console.log('error ===>', error);
            ShowToast('error', 'Unable to fetch items by category');
        ...
        <View style={[GeneralStyles.flex, { backgroundColor: colors.white }]}>
            ...
            <View style={styles.content}>
                ...
                <FlatList<MenuItem | HotelItem>
                    ...
                    columnWrapperStyle={route.params?.type === 'accomodation' ? undefined : styles.columnWrapper}
                        ...
                        if (route.params?.type === 'accomodation') {
                            ...
                            return (
                                <View style={{ marginBottom: 12 }}>
                                    <RecommendedCard
                                        image={hotel?.images?.[0] ? { uri: hotel.images[0] } : images.recommended_accomodation}
                        ...
                        return (
                            <View style={styles.cardWrapper}>
                                <FoodCardWithBorder
                                    ...
                                    id={menuItem?.id}
                                    hasFreeship={false}
                                    onPress={() => navigation.navigate('FoodDetails', { id: String(menuItem.id), name: menuItem.name, price: menuItem.price, image: menuItem.image || '', description: menuItem.description || '', category: menuItem.category || '', toppings: [], wishlistId: menuItem.wishlistId ?? null })}
