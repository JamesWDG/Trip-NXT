  ...
  const renderHorizontalTabs = ({
    ...
    return (
      <IconsWithTitle
        onPress={() => {
          if (item.navTo) {
            (navigation as any).navigate(item.navTo);
          } else {
            Alert.alert('working in progress');
          }
        }}
        image={item.image}
        title={item.name}
      ...
      <IconWithTitleAndDivider
        ...
        onPress={() => {
          if ('navigation' in item && item.navigation) {
            ...
            (navigation as any).navigate(item.navigation);
          }
        }}
      />
    );
...
export default FoodDrawerModal;
